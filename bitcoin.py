...
def generatePrivateKey():
    ...
    # This is the upper bound for private keys / secret exponents as specified in secp256k1
    privateKeyUpperBound = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
    privateKey = ""
    while True:
        ...
        for i in range(len(randArr)):
            # hex() prepends '0x', remove it and prepend 0 in front of any single digits
            # 5 -> 0x5 -> 5 -> 05
        ...
        if 1 < int(privateKey,16) < privateKeyUpperBound:
            break
...
def createAddressFromPassphrase(passphrase):
    ...
    signingKey = ecdsa.keys.SigningKey.from_secret_exponent(int(privateKey, 16), curve)
...
def example():
    ...
    signingKey = ecdsa.keys.SigningKey.from_secret_exponent(secretExponent, curve)
    verifyingKey = signingKey.get_verifying_key()
    x,y = getXY(verifyingKey.to_string())
